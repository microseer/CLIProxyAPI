...
func (h *OAuthWebHandler) refreshTokenData(ctx context.Context, storage *KiroTokenStorage) (*KiroTokenData, error) {
	...
	case storage.AuthMethod == "social":
		log.Debugf("OAuth Web: using Kiro social OAuth refresh endpoint")
		socialClient := NewSocialAuthClient(h.cfg)
		return socialClient.RefreshSocialToken(ctx, storage.RefreshToken)
